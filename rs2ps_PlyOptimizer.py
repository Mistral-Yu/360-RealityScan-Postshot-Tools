...
# -*- coding: utf-8 -*-
"""
rs2ps_PlyOptimizer (RealityScan to PostShot PLY optimizer)

- Load only (x, y, z) and (R, G, B) from PLY files.
- Optionally downsample on a voxel grid to make the cloud more uniform.
  - --target-points/-t automatically adjusts the voxel size to approach the requested count (prints detailed logs).
  - --target-percent/-r picks a point-count ratio.
  - --voxel-size/-v applies a fixed voxel size.
- Optionally append extra PLY files (after downsampling the base cloud).
- Output is a binary little-endian PLY (vertex: x, y, z, red, green, blue).

Default behaviour:
    representative point = original sample closest to the centroid
                           of all points gathered inside each voxel.
Additional strategies:
    center -> point closest to the voxel centre.
    first  -> earliest point encountered in the voxel (preserves ordering).
...
def voxel_downsample_by_size(
    ...
    *,
    ...
    `representative` selects how to pick the kept point for each voxel:
      - "centroid" -> point closest to the centroid of all points in the voxel (default).
      - "center"   -> point closest to the voxel center.
      - "first"    -> first point encountered in each voxel (preserves original order).
    """
    if xyz.shape[0] == 0:
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(
        prog="rs2ps-opt",
        description="RealityScan to PostShot PLY optimizer (XYZ+RGB load/save, voxel downsampling, append)"
    )
    ap.add_argument(
        "-i",
        "--in",
    ...
    if args.output is None:
        if ((args.voxel_size is not None and args.voxel_size > 0)
                ...
                or args.append_ply):
            print("[warn] --out not provided; ignoring downsample/append options.")
        else:
    ...
    if args.voxel_size is not None and args.voxel_size > 0:
        ...
        xyz, rgb = voxel_downsample_by_size(xyz, rgb, args.voxel_size, representative=args.keep_strategy)
